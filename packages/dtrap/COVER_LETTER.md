...
Paste the plain-text render, `COVER_LETTER.txt`, into the ScholarOne cover
letter field. The bracketed operator note under "Open-access fee" below is not
part of the letter and is not in the render.
...
co-editors-in-chief. Prof. Thomas Schreck's reply of 1 September 2026 gave the
view that, from the framing provided, the work fits, and that certainty is
...
in size or composition. The utility is practical: hash-verified stream
...
**Containment.** The work touches no live threat. It is offline analysis of
two public benchmark datasets, the Engelen-corrected CICIDS2017 release and
LITNET-2020. No malware samples, no operational systems, and no human subjects
are involved at any point. The datasets are not redistributed. The artifact
ships line-ending-normalized hashes so a reader verifies their own
reconstruction.

**Transparency.** Every measured value is generated by a script that writes an
archived run manifest in the same execution. A nine-check build gate fails
on any number without one, on inconsistent derived arithmetic, on text
typeset past the page measure, and on a figure whose inputs or generator
changed or whose plotted values disagree with the macro index. A
sentence-level claim ledger maps the abstract and introduction to generating
runs. A numbered corrected-incident log ships with the artifact, and the
artifact passes its own documented reproduction instructions inside a fresh
extraction. This manuscript is a corrected rebuild of work posted as
arXiv:2605.24696 v1 and v2. The corrected version was posted as v3 on 14
September 2026 and announced on 16 September 2026, with a correction note in
its arXiv Comments field. An adversarial audit of the archived artifacts
established the defects. The prior versions described a scoring rule the
released code did not implement, and their results were produced under the
composite construction the paper now studies as its labeled synthetic arm. I
would rather you have this history from me directly. The confidential
comments carry it in full.

**Open-access fee.** ACM confirmed on 6 August 2026 that its 100% geographic
open-access fee waiver applies to this paper and is applied automatically
during the eRights process. I can forward the confirmation on request.
[NOTE TO OPERATOR, not part of the pasted letter: the confirmation PDF is
C:\Users\CYBERWIZARD\Downloads\ACM_Waiver.pdf. Attach it only if the portal
shows a fee or waiver field, and only in that field. Never attach it in the
file-upload step. See SUBMISSION_CONSOLE.md, step 11.]
...
any venue. Its v2, posted 14 September 2026 and announced 16 September 2026,
adds a correction note that points to the corrected v3 of this work and to the
Zenodo 2.2.0 record. The manuscript's Companion Manuscript Disclosure section
describes the companion without its identifier. This manuscript is not under

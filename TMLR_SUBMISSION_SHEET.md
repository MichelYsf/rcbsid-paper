...
main text. The former Section 12, "Threats to Validity", is retitled
"Limitations and What Would Change Our Conclusions" and consolidates the
existing limitation content in one place; because the former Section 9 is now
an appendix, it is Section 11 in this build and "Relationship to Prior Versions
of This Work" is Section 10.
...
Paste verbatim. It is the compiled abstract with every macro resolved (430
whitespace-separated tokens, 2,784 characters, counted on the block below).
Verify against page 1 of the PDF before pasting.
...
The invitation marks this field hidden: OpenReview fills it from `authorids`.
If it is shown, it reads:
...
Leave empty. The invitation carries this optional ZIP field, labelled "Beyond
PDF", for interactive-webpage submissions only; it is not the supplementary
material and the artifact zip must not go here.

### 6. submission_length (displayed as "Submission Type")

Three options: Regular submission (no more than 12 pages of main content),
Long submission (more than 12 pages of main content), Beyond PDF submission
(pageless, webpage-style content). Select **Long submission (more than 12
pages of main content)**. The main
...
action editors for the submission" and that this information, like funding,
competing interests and human-subjects reporting, is not shown to reviewers.
The submission form has no field for it, so expect OpenReview to request it
after the submission is created; that step was not observable in this round
without an account, so if no request appears, write to the editors-in-chief
through the venue's contact address with the recommendation. Funding has no
field either; the competing-interests text above states there is none.
Recommend from the public board at jmlr.org/tmlr/editorial-board.html. The board lists each
...
| benchmarking, validity, distribution shift | Olawale Elijah Salaudeen |
...
the action editor is assigned, post this through the submission's comment
form with the readers set to the action editor (and the editors-in-chief if
offered) and not to reviewers; check the readers list before posting. The
paper promises the action editor this material: Section 10 says the
per-result-group account, the dated version history and the list of claims
each correction invalidates are supplied confidentially, so the note carries
all three. The anonymous PDF withholds the identifiers and the account.
...
This version is the rebuild from that audit; Section 10 and the Origin
...
not relied upon anywhere in this paper.

Dated version history: v1 posted 23 May 2026; v2 posted 25 June 2026 with a
reframed contribution statement; the adversarial review and audit that
established the defects ran in August 2026; the rebuild was completed and
reviewed in three pre-submission rounds in September 2026. The claims the
correction invalidates, each withdrawn here: that the released code implements
the run-length-reset posterior the earlier text describes (the evaluated score
is a maximum of a chi-square tail term and a weighted short-run posterior
mass); that the decision threshold is the cost-only rule described (it is the
prior-inclusive rule); that the quantity reported as latency in milliseconds
is a time (it is a count of records); that the reported operating points are
properties of the datasets and define attack-prevalence regimes (they are
properties of the assembled streams); and every table whose values had no
archived computational origin, including all results on the third dataset.
The corrected-incident log in the artifact records each correction with its
evidence and closure status.

A corrected v3 of the preprint, matching this submission, will be posted after
submission; TMLR's preprint policy permits this and the anonymous PDF names no
version.

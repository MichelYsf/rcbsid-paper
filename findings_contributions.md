...
Generating run: `s5_verified_contributions_20260909T060341_6108db2a`. Every count is a provenance macro.
...
**Action.** The evaluated system is **prequential global-Gaussian tail scoring**. Stage 6 has now run and did NOT rescue this: on the synthetic probe the alternative reset formulation makes P(r=0) respond (peak 1.000000 against a hazard of 0.001000), but on the stream it saturates the composed score - 92.7% of its values are exactly 0.25 - giving an AUC-ROC of 0.5096, which is near chance. The two variants are degenerate in different quantities. The verdict stands.

...
are carried forward unchanged. Because the full-stream grid exceeded the compute ceiling, grid points were evaluated on the first chronologically contiguous 40\% of the training split followed by the full validation split; the selected configuration was then re-run on the full stream for the reported test numbers. 8 grid point(s) crashed; each is logged with its error in the released results and excluded from selection.

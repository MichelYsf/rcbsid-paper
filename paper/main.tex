...
\SFourCicidsPrevShiftPp{} percentage points
...
two deterministic methods under test: a plain batch baseline (ECOD) is best
under true timestamp order and worst under the interleaving, while the
evaluated detector's ordering against it reverses exactly. On LITNET-2020,
the single ``\SFourLitnetPooledTestPrev\%\ rare-attack regime'' is an
identity of pooling: the three temporally disjoint captures span
\SFourLitnetSpamTestPrev\%\ to \SFourLitnetUdpFloodTestPrev\%\ prevalence
individually, and the pooled figure is their equal-budget mixture weight.
We further characterize the evaluated detector itself: its run-length
posterior is algebraically pinned to the hazard rate, so the system is
prequential global-Gaussian tail scoring rather than change-point detection,
and the textbook repair saturates the score and detects nothing.
Every number in this paper traces to an archived, hash-verified run manifest,
...
prevalence by tens of points, replaces the evaluated sample almost entirely,
and inverts which method wins.
...
prose intended; and every number in this paper was regenerated by a script
that wrote, in the same execution, an archived manifest recording the git
commit, configuration hash, input SHA-256, seed, environment hash, and output
path. A build gate fails on any number without such a manifest. The
...
\SFourLitnetPooledTestPrev\%\ prevalence equals the equal-budget mixture of
three captures spanning \SFourLitnetSpamTestPrev\%\ to
\SFourLitnetUdpFloodTestPrev\%; it is a property of the assembly, not of any
capture.
...
\SThreeChangePointBindingMeanValue{} where it binds. The textbook repair
...
nothing: both variants are degenerate, in opposite directions.
...
prominence as the rest: on the identical natural-order held-out slice, a
batch LOF reference scores \STwoLUnresampledLofMean{} AUC-PR against the
detector's \STwoLUnresampledProposedMean{} (Section~\ref{sec:sweep}); and the
...
\textbf{Streaming anomaly detection.} The baselines evaluated here are
established streaming detectors: Half-Space Trees~\cite{tan2011hst},
LODA~\cite{pevny2016loda}, KitNET~\cite{mirsky2018kitsune},
xStream~\cite{manzoor2018xstream}, RRCF~\cite{guha2016rrcf}, and streaming
isolation forest~\cite{ding2013iforestasd}, with LOF~\cite{breunig2000lof},
ECOD~\cite{li2023ecod}, and COPOD~\cite{li2020copod} as batch references.
...
Measured consequence: the eligible week's prevalence is
...
(median run length 2, 90th percentile 70, maximum 2522 on CICIDS2017;
essentially isolated single flows on LITNET). Among these datasets, in
...
exactly (held-out \SFourCicidsInterleavedTestAttacks{} attacks in 240{,}000
records), so the contrast is against the construction actually used, not a
strawman.
...
Monday–Thursday contribute 162{,}000 attack-free held-out rows. The
synthetic arm's Friday sub-slice is denser (77.7\%) than the natural arm's
...
rests on seeds reported in Section~\ref{sec:seeds}. One caveat binds ECOD's
...
\label{sec:seeds}
HST is the only stochastic method carrying numbers here. Its placement
...
held-out prevalence. Deterministic cells are identical across draws;
stochastic cells' spreads are in the archived findings.}
\label{tab:sweep}
...
both variants over the first 15{,}000 records of the ablation stream, and
...
\SFourCicidsPrevShiftPp{} points, replace two-thirds of the evaluated
sample, and invert which deterministic method wins. Pooling disjoint
...
history are archived and will be linked here upon acceptance. During
double-anonymous review, the artifact is available through the submission
...
shares parts of the underlying codebase and predates the audit that
produced the present version. The measurements reported here concerning the
implemented scoring rule (Section~\ref{sec:detector}) apply to that shared
lineage; a correction process for the companion manuscript is underway and
is disclosed to editors of both venues. The two papers report disjoint
result sets.
